<block-div>
  <block ref="block" class="block-div-inner" is-container={ true } on-div-class={ onDivClass } get-blocks={ getBlocks } get-element={ getElement }>
    <yield to="body">
      ...
      <div class="{ opts.block.div } { this.acl.validate('admin') && !opts.preview ? 'eden-dropzone' : '' } { 'empty' : !opts.getBlocks(opts.block.children).length }" data-placement={ opts.placement + '.children' }>
        <div each={ child, a in opts.getBlocks(opts.block.children) } no-reorder class={ child.class } data-is={ opts.getElement(child) } preview={ opts.preview } editing={ opts.editing } data-block={ child.uuid } data={ opts.getBlock(child) } block={ child } get-block={ opts.getBlock } on-editing={ opts.onEditing } on-add-block={ opts.onAddBlock } on-save={ opts.onSave } on-remove={ opts.onRemove } on-refresh={ opts.onRefresh } i={ a } placement={ opts.placement + '.children.' + a } />
      </div>
